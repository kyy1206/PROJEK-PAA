...
"""
    ...
    def _haluskan_rute_bundaran(self, rute):
        ...
        for i in range(len(rute) - 1):
            ...
            if self._titik_di_bundaran(p1) and self._titik_di_bundaran(p2):
                arc = self._buat_arc_bundaran(p1, p2)

                for titik in arc[1:]:
                    if self._jarak_titik(hasil[-1], titik) > 0.75:
                        hasil.append(titik)
            else:
                if self._jarak_titik(hasil[-1], p2) > 0.75:
                    hasil.append(p2)
